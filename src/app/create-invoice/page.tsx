"use client";
import React, { useState } from "react";
import Image from "next/image";
import Icon from "@/assets/icon.png";
import EditIcon from "@/assets/edit.png";
import MainComponent from "@/components/maincomponent";
import CreateInvoicecomp from "@/components/create-invoice";
import Templates from "@/components/templates";

const CreateInvoice = () => {
  const [tapchoice, setTapChoice] = useState("Templates");
  return (
    <MainComponent
      para="Create a invoice for your patients"
      firstcomp={
        <div className="xl:w-full lg:bg-white lg:w-[425px] w-full  rounded-[10px] py-5">
          <div className="flex text-sm px-7 space-x-8">
            <p
              onClick={() => setTapChoice("Templates")}
              className={`cursor-pointer ${
                tapchoice == "Templates" &&
                "underline underline-offset-[17.5px]"
              }`}
            >
              Templates
            </p>
            <p
              onClick={() => setTapChoice("Create Tnvoice")}
              className={` cursor-pointer ${
                tapchoice == "Create Tnvoice" &&
                "underline underline-offset-[17.5px]"
              }`}
            >
              Create Invoice
            </p>
          </div>
          <hr className="mt-3 h-[2px] bg-[#4C4D4F33]/20" />
          {/* chnagingcomp */}
          {tapchoice == "Templates" ? <Templates /> : <CreateInvoicecomp />}
        </div>
      }
      secondcomp={
        <div className="bg-white lg:w-[377px] md:w-[690px] w-full rounded-[10px] px-4 pt-4 lg:pb-[20px] pb-60 xs:pb-20">
          <div className="flex items-center justify-between">
            <p className="lg:text-sm md:text-[27.74px] xs:text-[14.97px]">
              Preview Invoice
            </p>
            <div className="flex space-x-2">
              <div className="lg:w-[38px] lg:h-[38px] bg-[#F8F8F8] flex items-center justify-center rounded-lg md:w-[75.29px] md:h-[75.29px] xs:w-[40.62px] xs:h-[42.62px]">
                <Image src={EditIcon} alt="" />
              </div>
              <div className="lg:w-[38px] lg:h-[38px] bg-[#F8F8F8] flex items-center justify-center rounded-lg md:w-[75.29px] md:h-[75.29px] xs:w-[40.62px] xs:h-[42.62px]">
                <Image src={Icon} alt="" />
              </div>
            </div>
          </div>
          <div className="border border-[#4C4D4F33]/20 mt-5 rounded-[10px] p-4">
            <Image
              width={90.67}
              height={16}
              className="lg:w-[90.67px] lg:h-[16px] md:w-[179.65px] md:h-[31.7px] xs:w-[96.92px] xs:h-[17.1px]"
              src="/logo.png"
              alt=""
            />
            <div className="flex lg:text-[10px] items-center gap-1 mt-5 md:text-[19.81px] xs:text-[10.69px]">
              SUMMERY <hr className="w-full" />
            </div>
            <div className="lg:text-xs flex space-x-14 mt-6 md:text-[23.78px] xs:text-[12.83px]">
              <p>Dr. Vihang</p>
              <p>Jane Doe</p>
            </div>
            <div className="border-[1px] border-[#4C4D4F33]/20 rounded-[5px] overflow-hidden mt-7 lg:w-[305px]">
              {" "}
              <table className="border-collapse   overflow-hidden w-full lg:text-[10px] md:text-[19.81px] xs:text-[10.69px]">
                <thead className=" text-left bg-black text-white">
                  <tr className="">
                    <th className=" pl-3 py-4">item</th>
                    <th className="  py-4">Rate</th>
                    <th className="  py-4">Qty</th>
                    <th className="  py-4">Amount</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="">
                    <td className=" pl-2 py-3">Vitamin A Night Serum</td>
                    <td className="  py-2">$90</td>
                    <td className="  py-2">2</td>
                    <td className="  py-2">$180</td>
                  </tr>
                  <tr className="">
                    <td className=" pl-3 py-2">Vitamin A Night Serum</td>
                    <td className="  py-2">$90</td>
                    <td className="  py-2">2</td>
                    <td className="  py-2">$180</td>
                  </tr>
                  <tr className="">
                    <td className=" pl-3 py-2">Vitamin A Night Serum</td>
                    <td className="  py-2">$90</td>
                    <td className="  py-2">2</td>
                    <td className="  py-2">$180</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div className="flex lg:text-[10px] md:text-[19.81px] space-x-10 mt-2 justify-end px-2 xs:text-[10.69px]">
              <div>
                <p className="font-medium">Subtotal</p>
                <p className="text-[#4C4D4F80]/40">tax 5%</p>
                <p className="text-[#4C4D4F80]/40">Discount 10%</p>
              </div>
              <div>
                <p className="font-medium">$540.00</p>
                <p className="text-[#4C4D4F80]/40">$00.00</p>
                <p className="text-[#4C4D4F80]/40">$00.00</p>
              </div>
            </div>
            <hr className="mt-2 " />
            <div className="lg:text-[10px] md:text-[19.81px] flex space-x-10 justify-end mt-3 px-2 xs:text-[10.69px]">
              <p className="font-semibold">total</p>
              <p className="font-semibold">$540.00</p>
            </div>
            <div className="bg-[#F8F8F8] lg:text-[10px] md:text-[19.81px] p-4 mt-4 xs:text-[10.69px]">
              <h5>Notes/Terms</h5>
              <p className="text-[#00000080]/30">
                Lorem ipsum dolor sit amet. Sit eaque assumenda qui.
              </p>
            </div>
          </div>
          {tapchoice == "Templates" ? (
            <div className="flex flex-col space-y-2 text-[28.59px] lg:text-sm  mt-20 xs:mt-10 xs:text-[16.04px]">
              <button className="text-[#4C4D4F] border border-[#4C4D4F] rounded-full text-center flex justify-center items-center lg:w-[169px] lg:h-[48px] mx-auto sm:w-[375.81px] sm:h-[98.04px] xs:w-[210.84px] xs:h-[55px]">
                Save
              </button>
              <button className="text-white bg-black rounded-full text-center flex justify-center items-center lg:w-[169px] lg:h-[48px] mx-auto sm:w-[375.81px] sm:h-[98.04px] xs:w-[210.84px] xs:h-[55px]">
                Save & Send
              </button>
            </div>
          ) : (
            <div className="flex flex-col space-y-2 text-[28.59px] lg:text-sm  lg:flex-row lg:space-y-0 lg:mt-[349px] mt-20 xs:mt-10 xs:text-[16.04px]">
              <button className="text-[#4C4D4F] border border-[#4C4D4F] rounded-full text-center flex justify-center items-center lg:w-[148px] lg:h-[48px] mx-auto sm:w-[375.81px] sm:h-[98.04px] xs:w-[210.84px] xs:h-[55px]">
                Save
              </button>
              <button className="text-white bg-black rounded-full text-center flex justify-center items-center lg:w-[169px] lg:h-[48px] mx-auto sm:w-[375.81px] sm:h-[98.04px] xs:w-[210.84px] xs:h-[55px]">
                Save & Send
              </button>
            </div>
          )}
        </div>
      }
    />
  );
};

export default CreateInvoice;
